...
public abstract class ModRaider extends PatrollingMonster {
    ...
    @Nullable
    protected ModRaid raid;
    private int wave;
    private boolean canJoinRaid;
    private int ticksOutsideRaid;

    protected ModRaider(EntityType<? extends ModRaider> pEntityType, Level pLevel) {
    ...
    protected void registerGoals() {
        super.registerGoals();
        this.goalSelector.addGoal(1, new ModRaider.ObtainRaidLeaderBannerGoal<>(this));
        this.goalSelector.addGoal(3, new PathfindToRaidGoal<>(this));
        this.goalSelector.addGoal(4, new ModRaider.RaiderMoveThroughVillageGoal(this, 1.05F, 1));
        this.goalSelector.addGoal(5, new ModRaider.RaiderCelebration(this));
    }
    ...
    public void aiStep() {
        if (this.level() instanceof ServerLevel && this.isAlive()) {
            ModRaid raid = this.getCurrentRaid();
            if (this.canJoinRaid()) {
                if (raid == null) {
    ...
    public void setCurrentRaid(@Nullable ModRaid pRaid) {
        this.raid = pRaid;
    }

    @Nullable
    public ModRaid getCurrentRaid() {
        return this.raid;
    }
    ...
    protected class HoldGroundAttackGoal<T extends ModRaider> extends Goal {
        ...
        public HoldGroundAttackGoal(T pMob, final float pHostileRadiusSquare) {
            this.mob = pMob;
    ...
    public class ObtainRaidLeaderBannerGoal<T extends ModRaider> extends Goal {
        private final T mob;
        ...
        public boolean canUse() {
            ...
            if (this.mob.hasActiveRaid()
                ...
                ModRaider raider = raid.getLeader(this.mob.getWave());
                if (raider == null || !raider.isAlive()) {
                    List<ItemEntity> list = this.mob
                            .level()
                            .getEntitiesOfClass(ItemEntity.class, this.mob.getBoundingBox().inflate(16.0, 8.0, 16.0), ModRaider.ALLOWED_ITEMS);
                    if (!list.isEmpty()) {
        ...
        public void tick() {
            if (this.mob.getNavigation().getTargetPos().closerToCenterThan(this.mob.position(), 1.414)) {
                List<ItemEntity> list = this.mob.level().getEntitiesOfClass(ItemEntity.class, this.mob.getBoundingBox().inflate(4.0, 4.0, 4.0), ModRaider.ALLOWED_ITEMS);
                if (!list.isEmpty()) {
    ...
    public class RaiderCelebration extends Goal {
        ...
        public void tick() {
            if (!this.mob.isSilent() && this.mob.random.nextInt(this.adjustedTickDelay(100)) == 0) {
                ModRaider.this.makeSound(ModRaider.this.getCelebrateSound());
            }
    ...
    static class RaiderMoveThroughVillageGoal extends Goal {
        private final ModRaider modRaider;
        ...
        public RaiderMoveThroughVillageGoal(ModRaider pModRaider, double pSpeedModifier, int pDistanceToPoi) {
            this.modRaider = pModRaider;
        ...
        public boolean canUse() {
            this.updateVisited();
            return this.isValidRaid() && this.hasSuitablePoi() && this.modRaider.getTarget() == null;
        }

        private boolean isValidRaid() {
            return this.modRaider.hasActiveRaid() && !this.modRaider.getCurrentRaid().isOver();
        }

        private boolean hasSuitablePoi() {
            ServerLevel serverlevel = (ServerLevel)this.modRaider.level();
            BlockPos blockpos = this.modRaider.blockPosition();
            Optional<BlockPos> optional = serverlevel.getPoiManager()
                    .getRandom(p_219843_ -> p_219843_.is(PoiTypes.HOME), this::hasNotVisited, PoiManager.Occupancy.ANY, blockpos, 48, this.modRaider.random);
            if (optional.isEmpty()) {
                return false;
            } else {
        ...
        public boolean canContinueToUse() {
            return this.modRaider.getNavigation().isDone()
                    ? false
                    : this.modRaider.getTarget() == null
                    && !this.poiPos.closerToCenterThan(this.modRaider.position(), (double)(this.modRaider.getBbWidth() + (float)this.distanceToPoi))
                    && !this.stuck;
        }

        @Override
        public void stop() {
            if (this.poiPos.closerToCenterThan(this.modRaider.position(), (double)this.distanceToPoi)) {
        ...
        public void start() {
            super.start();
            this.modRaider.setNoActionTime(0);
            this.modRaider
                    .getNavigation()
        ...
        @Override
        public void tick() {
            if (this.modRaider.getNavigation().isDone()) {
                Vec3 vec3 = Vec3.atBottomCenterOf(this.poiPos);
                Vec3 vec31 = DefaultRandomPos.getPosTowards(this.modRaider, 16, 7, vec3, (float) (Math.PI / 10));
                if (vec31 == null) {
                    vec31 = DefaultRandomPos.getPosTowards(this.modRaider, 8, 7, vec3, (float) (Math.PI / 2));
                }
                ...
                this.modRaider.getNavigation().moveTo(vec31.x, vec31.y, vec31.z, this.speedModifier);
            }
        }

...
public class EmpireCrossbowmenArmorLayer extends GeoRenderLayer<EmpireCrossbowmen> {

    private final EmpireCrossbowmenArmorModel armorModel = new EmpireCrossbowmenArmorModel();

    public EmpireCrossbowmenArmorLayer(GeoRenderer<EmpireCrossbowmen> renderer) {
        super(renderer);
    }

    @Override
    public void render(PoseStack poseStack, EmpireCrossbowmen animatable, BakedGeoModel bakedModel, RenderType renderType,
                       MultiBufferSource bufferSource, VertexConsumer buffer, float partialTick, int packedLight, int packedOverlay) {

        BakedGeoModel armorBakedModel = armorModel.getBakedModel(armorModel.getModelResource(animatable));

        matchBonePose(bakedModel, armorBakedModel, "root");

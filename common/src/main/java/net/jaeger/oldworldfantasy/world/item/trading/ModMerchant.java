    ...
    default void openTradingScreen(Player pPlayer, Component pDisplayName, int pLevel) {
        ...
        if (optionalint.isPresent() && pPlayer instanceof ServerPlayer serverPlayer) {
            ...
            if (!merchantoffers.isEmpty()) {
                ModNetworking.sendToPlayer(serverPlayer, new MerchantOfferPacket(optionalint.getAsInt(),
                        merchantoffers, pLevel, this.getVillagerXp(), this.showProgressBar(), this.canRestock()));
            }
        }
    }

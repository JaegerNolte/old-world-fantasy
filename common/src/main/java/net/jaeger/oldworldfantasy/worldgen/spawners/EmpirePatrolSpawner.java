...
import static net.jaeger.oldworldfantasy.config.spawndata.PatrolSpawnConfig.EMPIRE_WEIGHTS;

public class EmpirePatrolSpawner implements CustomSpawner {
    private int NEXT_TICK = 0;
    private final int PATROL_COOLDOWN = 6000;
    private final EntityType<EmpireCaptain> PATROL_LEADER = ModEntities.EMPIRE_CAPTAIN.get();
            ...
            if (this.NEXT_TICK > 0) {
                ...
                if (!pLevel.isDay()) {
                    ...
                    if (j < 1) {
                        ...
                        OldWorldFantasy.LOG.info("Selected player at {}", player.blockPosition());

                        if (player.isSpectator()) {

                            if (player.isSpectator()) {
                                OldWorldFantasy.LOG.info("STOP: player is spectator");
                                return 0;
                            }
                            ...
                            OldWorldFantasy.LOG.info("Potential patrol position: {}", mutableblockpos);
    ...
    private boolean spawnPatrolMember(ServerLevel pLevel, BlockPos pPos, RandomSource pRandom, boolean pLeader) {
        ...
        if (pLeader) {
            ...
            entityType = getRandomEmpire(pRandom);
        }
        ...
        if (!NaturalSpawner.isValidEmptySpawnBlock(pLevel, pPos, blockstate, blockstate.getFluidState(), entityType)) {
            ...
            if (patrollingMonster != null) {
                ...
                OldWorldFantasy.LOG.info("Spawned {}{} at {}", entityType, pLeader ? " [LEADER]" : "", pPos);
                return true;
            } else {
                return false;
            }
    ...
    private static EntityType<? extends PatrollingMonster> getRandomEmpire(RandomSource random) {
        int totalWeight = EMPIRE_WEIGHTS.stream().mapToInt(SpawnDataHelper::weight).sum();
        int roll = random.nextInt(totalWeight);
        for (SpawnDataHelper data : EMPIRE_WEIGHTS) {
            roll -= data.weight();
            if (roll < 0) {
                return data.entityType();
            }
        }
        return EMPIRE_WEIGHTS.getFirst().entityType();
    }
}
